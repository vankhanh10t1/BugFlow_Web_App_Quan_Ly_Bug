...
export async function PATCH(request: Request, { params }: Context) { try { assertSameOriginRequest(request); const actor = await requireActiveUser(); const parsed = bugUpdateSchema.safeParse(await request.json()); if (!parsed.success) throw new AppError("VALIDATION_ERROR", "Invalid bug data", 400); await enforceUserMutationLimit("bug:update", actor.id, 30); return apiSuccess(await updateBug((await params).bugId, actor, parsed.data), "Bug updated successfully"); } catch (error) { return apiError(error); } }
