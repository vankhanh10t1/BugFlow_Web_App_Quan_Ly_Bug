...
export async function POST(request: Request) { try { assertSameOriginRequest(request); const actor = await requireActiveUser(); const parsed = bugInputSchema.safeParse(await request.json()); if (!parsed.success) throw new AppError("VALIDATION_ERROR", "Invalid bug data", 400); await enforceUserMutationLimit("bug:create", actor.id); return apiSuccess(await createBug(actor, parsed.data), "Bug created successfully", 201); } catch (error) { return apiError(error); } }
