...
export async function DELETE(_: Request, { params }: Context) {
  try { const actor = await requireActiveUser(); const ids = await params; await removeProjectMember(ids.projectId, ids.memberId, actor); return apiSuccess(null, "Member removed successfully"); } catch (error) { return apiError(error); }
}
