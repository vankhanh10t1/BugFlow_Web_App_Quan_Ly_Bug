...
export default async function BugsPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <><div className="flex items-end justify-between gap-4"><div><p className="text-sm font-medium text-blue-600">Theo dõi lỗi</p><h1 className="mt-1 text-3xl font-semibold tracking-tight">Danh sách lỗi</h1><p className="mt-2 text-slate-600">Tìm kiếm và phân loại lỗi trong các dự án của bạn.</p></div><NewBugLink /></div><BugList actor={actor} query={query} raw={raw} /></>;
}
