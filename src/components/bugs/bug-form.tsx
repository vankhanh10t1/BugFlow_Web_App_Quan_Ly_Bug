...
type BugDefaults = { id: string; title: string; description: string; reproductionSteps: string | null; expectedResult: string | null; actualResult: string | null; environment: string | null; browser: string | null; operatingSystem: string | null; applicationVersion: string | null; priority: string; severity: string; dueDate: Date | null; project?: { id: string }; component?: { id: string } | null; version?: { id: string } | null; labels?: { id: string }[] };
type Metadata = { labels: { id: string; name: string; projectId: string }[]; components: { id: string; name: string; projectId: string }[]; versions: { id: string; name: string; projectId: string }[] };
...
export function BugForm({ projects = [], bug, metadata = { labels: [], components: [], versions: [] }, attachmentLimits = { maxFiles: 5, maxSizeMb: 10 } }: { projects?: ProjectOption[]; bug?: BugDefaults; metadata?: Metadata; attachmentLimits?: { maxFiles: number; maxSizeMb: number } }) {
  ...
  return <form action={formAction} className="space-y-5 rounded-xl border bg-white p-6 shadow-sm">
    ...
    <div className="grid gap-5 sm:grid-cols-3"><div><label className="text-sm font-medium" htmlFor="labelIds">Nhãn</label><select id="labelIds" name="labelIds" defaultValue={bug?.labels?.[0]?.id ?? ""} className="mt-1 h-11 w-full rounded-lg border bg-white px-3 text-sm"><option value="">Không có nhãn</option>{metadata.labels.filter((x) => !bug?.project || x.projectId === bug.project.id).map((x) => <option key={x.id} value={x.id}>{x.name}</option>)}</select></div><div><label className="text-sm font-medium" htmlFor="componentId">Thành phần</label><select id="componentId" name="componentId" defaultValue={bug?.component?.id ?? ""} className="mt-1 h-11 w-full rounded-lg border bg-white px-3 text-sm"><option value="">Chưa chọn</option>{metadata.components.filter((x) => !bug?.project || x.projectId === bug.project.id).map((x) => <option key={x.id} value={x.id}>{x.name}</option>)}</select></div><div><label className="text-sm font-medium" htmlFor="versionId">Phiên bản</label><select id="versionId" name="versionId" defaultValue={bug?.version?.id ?? ""} className="mt-1 h-11 w-full rounded-lg border bg-white px-3 text-sm"><option value="">Chưa chọn</option>{metadata.versions.filter((x) => !bug?.project || x.projectId === bug.project.id).map((x) => <option key={x.id} value={x.id}>{x.name}</option>)}</select></div></div>
