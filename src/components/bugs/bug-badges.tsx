...
export function StatusBadge({ value }: { value: BugStatus }) { return <span className={`rounded-full px-2.5 py-1 text-xs font-medium ${statusStyles[value]}`}>{value.replaceAll("_", " ")}</span>; }
export function PriorityLabel({ value }: { value: BugPriority }) { return <span className={`text-xs font-semibold ${priorityStyles[value]}`}>{value}</span>; }
export function SeverityLabel({ value }: { value: BugSeverity }) { return <span className={`text-xs font-semibold ${severityStyles[value]}`}>{value}</span>; }
