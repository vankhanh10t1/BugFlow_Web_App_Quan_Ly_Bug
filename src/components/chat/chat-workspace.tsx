...
async function json<T>(url: string, init?: RequestInit) { const response = await fetch(url, init); const body = await response.json() as Envelope<T>; if (!response.ok) throw new Error(body.message || "Yêu cầu không thành công"); return body.data; }

export function ChatWorkspace({ currentUserId, initialConversationId }: { currentUserId: string; initialConversationId?: string }) {
