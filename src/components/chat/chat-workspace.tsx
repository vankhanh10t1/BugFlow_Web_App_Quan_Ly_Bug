...
export function mergeChatMessages(serverMessages: Message[], pendingMessages: Message[]) {
...
function ChatWorkspaceReady({ currentUserId, currentUserRole, initialConversationId, initialConversations, initialCandidates }: { currentUserId: string; currentUserRole: string; initialConversationId?: string; initialConversations: Conversation[]; initialCandidates: Candidates }) {
  ...
  const reaction = useMutation({
    mutationFn: ({ messageId, emoji, remove }: { messageId: string; emoji?: string; remove: boolean }) => json<{ reactions: ReactionSummary[]; myReaction: string | null }>(`/api/chat/messages/${messageId}/reactions`, {
    ...
    onSuccess: (summary, variables) => {
      client.setQueryData<{ items: Message[]; nextCursor: string | null }>(["chat", "messages", activeId], (current) => current ? {
        ...current,
        items: current.items.map((message) => message.id === variables.messageId ? { ...message, ...summary } : message),
      } : current);
      setNotice("");
    },
    onError: (error) => setNotice(error.message),
  ...
  const allMessages = useMemo(() => {
    return mergeChatMessages(messages.data?.items ?? [], pendingMessages);
  }, [messages.data?.items, pendingMessages]);
...
type MessageBubbleProps = { message: Message; mine: boolean; selected: boolean; toggleSelected: () => void; copy: () => void; act: (action: string) => void; react: (emoji: string) => void; reacting: boolean; retry: () => void };
function messageVisualKey(message: Message) {
  return [message.id, message.content, message.type, message.priority, message.sticker, message.gifUrl, message.gifWidth, message.gifHeight, message.attachmentUrl, message.attachmentMime, message.reminderAt, message.pinnedAt, message.recalledAt, message.marked, message.deliveryStatus, message.pending, message.failed, message.myReaction, JSON.stringify(message.reactions ?? [])].join("|");
}
const MessageBubble = memo(function MessageBubble({ message, mine, selected, toggleSelected, copy, act, react, reacting, retry }: MessageBubbleProps) {
  ...
  return <div className={`group flex items-start gap-2 ${mine ? "justify-end" : "justify-start"}`}>
    ...
    <article className={`relative max-w-[85%] rounded-2xl px-4 py-3 text-sm shadow-sm ${mine ? "bg-blue-600 text-white" : "border bg-white text-slate-800"} ${message.priority === "URGENT" ? "ring-2 ring-red-400" : message.priority === "IMPORTANT" ? "ring-2 ring-amber-300" : ""}`}>
      ...
      {!recalled && !message.pending ? <div className="relative mt-1 flex flex-wrap items-center gap-1">
        {(message.reactions ?? []).map((item) => <button type="button" key={item.emoji} disabled={reacting} onClick={() => react(item.emoji)} aria-pressed={item.reactedByMe} className={`rounded-full border px-2 py-0.5 text-xs ${item.reactedByMe ? "border-blue-300 bg-blue-50 text-blue-700" : mine ? "border-blue-400 bg-blue-500 text-white" : "bg-white text-slate-700"}`}>{item.emoji} {item.count}</button>)}
        <button type="button" disabled={reacting} onClick={() => setShowReactions((value) => !value)} aria-label="Thả cảm xúc" className={`rounded-full border px-1.5 py-0.5 text-xs opacity-70 transition group-hover:opacity-100 focus:opacity-100 ${mine ? "border-blue-400" : "bg-white"}`}>☺</button>
        {showReactions ? <div className="absolute bottom-full left-0 z-30 mb-1 flex gap-1 rounded-full border bg-white p-1.5 shadow-xl">{defaultReactions.map((emoji) => <button type="button" key={emoji} disabled={reacting} onClick={() => { react(emoji); setShowReactions(false); }} aria-label={`Thả cảm xúc ${emoji}`} className={`grid size-8 place-items-center rounded-full text-lg hover:bg-slate-100 ${message.myReaction === emoji ? "bg-blue-100" : ""}`}>{emoji}</button>)}</div> : null}
      </div> : null}
...
}, (previous, next) => previous.mine === next.mine && previous.selected === next.selected && previous.reacting === next.reacting && messageVisualKey(previous.message) === messageVisualKey(next.message));
...
export function regionalFlagCode(flag: string) {
...
function StableGif({ src, width, height }: { src: string; width: number; height: number }) {
  ...
  return <img src={src} alt="GIF từ GIPHY" width={width} height={height} loading="lazy" decoding="async" className="mt-2 max-h-72 max-w-full rounded-lg object-contain" />;
}
