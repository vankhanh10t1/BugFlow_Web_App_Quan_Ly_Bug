...
- Project `VIEWER` members are read-only. Chat uses a lightweight searchable/category picker built from `@emoji-mart/data` (without the React-18-only wrapper), a Windows/Chromium country-flag font polyfill, and GIPHY React Components for GIF search/sending; it also supports stickers, Cloudinary images/video/files, browser-authorized screenshots, important/urgent messages, pin/mark/multi-select/recall/delete-for-me actions, and a conversation-information panel.
